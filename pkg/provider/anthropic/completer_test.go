...
func TestCompleterStreaming(t *testing.T) {
	...
	if reasonings[0].Text != "let me think" || reasonings[0].Signature != "SIG_1" || reasonings[0].Kind == provider.ReasoningKindRedacted {
		t.Errorf("thinking: %+v", reasonings[0])
	}
	if reasonings[1].Signature != "BLOB" || reasonings[1].Kind != provider.ReasoningKindRedacted {
...
func TestConvertRequest_RedactedThinking(t *testing.T) {
	...
	want := []string{"text", "thinking", "redacted_thinking"}
	if len(types) != len(want) {
		t.Fatalf("blocks: got %v", types)
	}

	var redacted map[string]any
	for _, c := range content {
		if m := c.(map[string]any); m["type"] == "redacted_thinking" {
			redacted = m
		}
	}

	if redacted == nil {
		t.Fatalf("expected redacted_thinking block, got %v", types)
	}
